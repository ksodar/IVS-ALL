	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('rgbfeaturedcategory', $this->request->post);
			} else {
		...
		$data['entry_carousel_status'] = $this->language->get('entry_carousel_status');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/rgbfeaturedcategory', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/rgbfeaturedcategory', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/rgbfeaturedcategory', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/rgbfeaturedcategory', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['carousel_status'])) {
			...
			$data['carousel_status'] = $module_info['carousel_status'];
		} else {
			$data['carousel_status'] = '';
		}
		...
		$this->response->setOutput($this->load->view('extension/module/rgbfeaturedcategory.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/rgbfeaturedcategory')) {
