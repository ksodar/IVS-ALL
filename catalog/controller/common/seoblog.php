...
if (!class_exists('ControllerCommonSeoBlog')) {
	class ControllerCommonSeoBlog extends Controller {
		...
		public function __construct($registry) {
			...
			if ((isset($this->data['settings_general']['seocms_url_secure']) && $this->data['settings_general']['seocms_url_secure'] == 'https' && $this->data['settings_general']['seocms_url_secure'] != 'http') || ((isset($_SERVER['HTTPS']) && (strtolower($_SERVER['HTTPS']) == 'on' || $_SERVER['HTTPS'] == '1')) || (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && (strtolower($_SERVER['HTTP_X_FORWARDED_PROTO']) == 'https') || (!empty($_SERVER['HTTP_X_FORWARDED_SSL']) && strtolower($_SERVER['HTTP_X_FORWARDED_SSL']) == 'on'))) && (isset($this->data['settings_general']['seocms_url_secure']) && (isset($this->data['settings_general']['seocms_url_secure']) && $this->data['settings_general']['seocms_url_secure'] != 'http'))) {
		...
		public function router() {
			if (!$this->registry->get('request_blog_router')) {
				...
				if ($this->flag_router == 'search' || $this->flag_router == 'record' || $this->flag_router == 'blog') {
					if (!$this->registry->get('fblogwork')) {
						$this->registry->set('fblogwork', true);
						if (SC_VERSION > 15) {
		...
		public function rewrite($link) {
			...
			if ($this->cache_rewrite_flag) {
				...
				if (isset($this->cache_rewrite_data[$cache_hach_link])) {
					$link = $this->cache_rewrite_data[$cache_hach_link];
					if (!$is_blog_work) {
						$this->config->set("blog_work", false);
					}
					return $link;
			...
			if ($seocms_force_flag == false) {
				if (isset($url_data['path']) && $url_data['path']) {
					if ($url_data['path'] != $this->url_search) {
						...
						if ($this->cache_rewrite_flag) {
							if (is_array($this->cache_rewrite_data)) {
		...
		private function validate() {
			...
			$url = urldecode(str_replace('&amp;', '&', ltrim($_SERVER['REQUEST_URI'], '/')));
			$seo = urldecode(str_replace('&amp;', '&', str_replace($this->config_url, '', $this->url->link($this->request->get['route'], $this->getQueryString(array(
				'route',
				'_route_',
				'site_language'
			)), 'SSL'))));
